...
class SoleonDashboard(object):
    '''
    A soleon testing dashboard for MAVProxy
    '''

    def __init__(self, title="MAVProxy: Soleon Aircontroller Dashboard"):
        self.title = title
    ...
    def child_task(self):
        ...
        from MAVProxy.modules.lib.wxsoleondash_ui import SoleonDashboardFrame

        # create the wx application and pass self as the state
        app = wx.App()
        app.frame = SoleonDashboardFrame(state=self, title=self.title, size=(800, 300))
...
if __name__ == "__main__":
    '''A stand alone test for the soleon dashboard'''

    multiproc.freeze_support()
    soleon_dash = SoleonDashboard()
    while soleon_dash.is_alive():
        print('soleon dashboard is alive')
        time.sleep(0.5)
